...
async function checkEntities() {
    ...
    try {
        ...
        const entityMetadatas = data_source_1.AppDataSource.entityMetadatas;
        ...
        for (const entityName of entitiesToCheck) {
            ...
            if (entity) {
                ...
                try {
                    const tableExists = await data_source_1.AppDataSource.query(`SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = '${entity.tableName}')`);
                    if (tableExists[0].exists) {
                        console.log(`   📊 Таблица существует в базе данных`);
                        // Получаем количество записей
                        const countResult = await data_source_1.AppDataSource.query(`SELECT COUNT(*) as count FROM "${entity.tableName}"`);
        ...
        const tasksInfo = await data_source_1.AppDataSource.query(`
        ...
        if (tasksInfo.length > 0) {
            ...
            const constraints = await data_source_1.AppDataSource.query(`
