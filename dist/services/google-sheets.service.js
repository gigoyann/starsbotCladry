...
class GoogleSheetsService {
    ...
    async updateUserBalanceInSheets(telegramId, balance) {
        try {
        ...
        try {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                try {
                    ...
                    const [dbIdStr, // A: ID из БД (не используем)
                    ...
                    if (!user) {
                        ...
                        const normalizedStatus = this.normalizeUserStatus(status);
                        const hasChanges = user.stars !== stars ||
    ...
    async syncWithdrawalsFromSheets() {
        try {
            ...
            for (const row of rows) {
                try {
                    ...
                    if (!id || !telegramId || !amountStr || !status)
                        continue;
                    const withdrawalId = parseInt(id);
                    const userTelegramId = parseInt(telegramId);
                    const amount = parseInt(amountStr);
                    const normalizedStatus = this.normalizeStatus(status);
                    if (isNaN(withdrawalId) || isNaN(userTelegramId) || isNaN(amount) || !normalizedStatus)
                        continue;
                    // Находим пользователя
                    const user = await userRepository.findOne({
                        where: { telegramId: userTelegramId }
                    ...
                    if (!withdrawal) {
                        ...
                        const hasChanges = withdrawal.status !== normalizedStatus ||
        ...
        try {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                try {
                    ...
                    const [telegramIdStr, // B: Telegram ID
                    , // C: Имя (пропускаем)
