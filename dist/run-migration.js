...
async function runAllMigrations() {
    try {
        ...
        const executedMigrations = await queryRunner.query('SELECT name FROM migrations ORDER BY executed_at');
            ...
            if (!executedNames.includes(migrationName)) {
                ...
                try {
                    ...
                    if (migrationModule[migrationName]) {
                        ...
                        const classes = Object.values(migrationModule).filter((item) => item.prototype && item.prototype.constructor);
                        MigrationClass = classes[0];
                    }
                    if (MigrationClass) {
                        ...
                        await queryRunner.query('INSERT INTO migrations (name) VALUES ($1)', [migrationName]);
