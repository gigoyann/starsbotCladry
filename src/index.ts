...
class StarBot {
    ...
    private readonly MIN_REFERRALS_FOR_WITHDRAWAL = 5;
    private readonly GUESS_GAME_BET = 5; // Ставка для игры
    private activeGames = new Map<number, number>();
    private lastButtonPress = new Map<number, number>();
    private readonly GAME_TIMEOUT = 10000; // 10 секунд максимум на игру
    private readonly DEBOUNCE_TIME = 1000; // 1 секунда между нажатиями
    ...
    private releaseGameLock(userId: number): void {
        this.activeGames.delete(userId);
    ...
    private async withGameLock(ctx: BotContext, gameCallback: Function, betAmount?: number): Promise<void> {
        const userId = ctx.from!.id;
    ...
    private async safeEditMessage(ctx: BotContext, newText: string, keyboard?: any): Promise<boolean> {
        try {
    ...
    private async showTasksMenu(ctx: BotContext): Promise<void> {
        try {
            ...
            const completedTaskIds: number[] = completedTasks.map(ut => ut.taskId);
    ...
    private async getNextTaskForUser(userId: number): Promise<Task | null> {
        try {
            ...
            const completedTaskIds: number[] = completedTasks.map(ut => ut.taskId);
            const pendingTaskIds: number[] = pendingTasks.map(ut => ut.taskId);

            const allUsedTaskIds = [...completedTaskIds, ...pendingTaskIds];

            const availableTasks = allTasks.filter(task =>
                !allUsedTaskIds.includes(task.id)
            );
    ...
    private async buildTrackingUrl(originalUrl: string, clickId: string, userId: number): Promise<string> {
    ...
    private async createTaskViaAdmin(ctx: BotContext, type: string, title: string, description: string, reward: number, url?: string, channel?: string, bot?: string): Promise<void> {
        try {
            ...
            try {
                ...
                if (ctx.callbackQuery?.message) {
                    await ctx.editMessageText(
                        ...
                        `💰 Нужно: ${betAmount} ⭐\n` +
                        `⭐ У вас: ${user.stars} ⭐\n\n` +
                        `💡 Получите больше звезд через рефералов или подождите ежедневный бонус!`,
                        { parse_mode: 'Markdown', reply_markup: keyboard }
                    );
                }
            } catch (e) {
    ...
    private setupBroadcastCommands() {
        ...
        this.bot.command('confirmbroadcast', async (ctx) => {
            ...
            await ctx.reply(
                ...
                `📈 Успешность: ${Math.round((result.success / userIds.length) * 100)}%`,
                { parse_mode: 'Markdown' }
            );
    ...
    private setupAllHandlers() {
        ...
        this.bot.command('check_ref', async (ctx) => {
            ...
            await ctx.reply(
                ...
                `⭐ Звезд с рефералов: ${(user.referralsCount || 0) * 5}`,
                { parse_mode: 'Markdown' }
            );
        });
    ...
    private async showGuessNumberMenu(ctx: BotContext): Promise<void> {
        try {
            const user = ctx.user!;
            ...
            if (user.stars < betAmount) {
                ...
                if (ctx.callbackQuery?.message) {
                    await ctx.editMessageText(
                        ...
                        `💰 Ставка: ${betAmount} ⭐\n` +
                        `⭐ У вас: ${user.stars} ⭐\n\n` +
                        `💡 Получите больше звезд через рефералов!`,
                        { parse_mode: 'Markdown', reply_markup: keyboard }
                    );
                }
                return;
    ...
    private async showChannelsToSubscribe(ctx: BotContext) {
        ...
        const checkButton = Markup.button.callback(
            '✅ Я подписался на все каналы',
            `check_subscription_${ctx.from!.id}`
        );
        buttons.push([checkButton]);

        await ctx.reply(
            '🎯 Добро пожаловать! Для начала работы необходимо подписаться на наши каналы:\n\n' +
    ...
    private async showGamesMenu(ctx: BotContext) {
        try {
            ...
            const keyboard = Markup.inlineKeyboard([
                [
                    Markup.button.callback('🎲 Кости (3⭐)', 'play_animated_dice'),
                    Markup.button.callback('🏀 Баскетбол (5⭐)', 'play_animated_basketball')
                ],
                [
                    Markup.button.callback('🎯 Дартс (4⭐)', 'play_animated_darts'),
                    // Markup.button.callback('⚽ Футбол (5⭐)', 'play_animated_football')
                ],
                [
                    Markup.button.callback('🎳 Боулинг (6⭐)', 'play_animated_bowling'),
                    Markup.button.callback('🎰 Слоты (10⭐)', 'play_animated_slots')
                ],
                [
                    Markup.button.callback('🎲 Угадайка (5⭐)', 'play_animated_guess'), // ← НОВАЯ ИГРА
                ],
                [
    ...
    private async showReferralsMenu(ctx: BotContext) {
        try {
            ...
            const keyboard = Markup.inlineKeyboard([
                [
                    Markup.button.callback('📋 Копировать ссылку', 'copy_referral_link'),
                    Markup.button.callback('📤 Поделиться', 'share_referral_link')
                ],
                [
                    Markup.button.callback('⬅️ Назад в меню', 'back_to_menu')
                ]
    ...
    private async copyReferralLink(ctx: BotContext) {
        try {
            ...
            await ctx.reply(
                ...
                `4. Друг получит 10⭐ за регистрацию`,
                { parse_mode: 'Markdown' }
            );
    ...
    private async notifyAdminAboutWithdrawal(user: User, amount: number, withdrawalId: number, referralsCount?: number) {
        try {
            if (this.adminId) {
                const message =
    ...
    private async showWithdrawMenu(ctx: BotContext) {
        try {
            ...
            const keyboard = Markup.inlineKeyboard([
                ...
                [
                    Markup.button.callback('150 ⭐', 'withdraw_150'),
                    Markup.button.callback('200 ⭐', 'withdraw_200'),
                ],
                [
                    Markup.button.callback('500 ⭐', 'withdraw_500'),
                    Markup.button.callback('Все ⭐', 'withdraw_all')
                ],
                [
                    Markup.button.callback('👥 Пригласить друзей', 'show_referrals')
                ],
                [
                    Markup.button.callback('⬅️ Назад в меню', 'back_to_menu')
                ]
    ...
    private async playAnimatedSlots(ctx: BotContext, betAmount: number): Promise<void> {
        const userId = ctx.from!.id;
        ...
        try {
            ...
            await this.showAnimatedGameResult(
                ctx, user, 'animated_slots', '🎰',
                slotValue, betAmount, winAmount, resultText
            );
    ...
    private async playGuessGame(ctx: BotContext, chosenNumber: number): Promise<void> {
        const userId = ctx.from!.id;
        const betAmount = this.GUESS_GAME_BET;
    ...
    private async showGuessGameResult(
        ctx: BotContext,
        user: User,
        diceValue: number,
        chosenNumber: number,
    ...
    private async playAnimatedDice(ctx: BotContext, betAmount: number): Promise<void> {
        const userId = ctx.from!.id;
        ...
        try {
            ...
            // Показываем результат
            await this.showAnimatedGameResult(
                ctx, user, 'animated_dice', '🎲',
                diceValue, betAmount, winAmount, resultText
            );
    ...
    private async playAnimatedDarts(ctx: BotContext, betAmount: number): Promise<void> {
        const userId = ctx.from!.id;
        ...
        try {
            ...
            // Показываем результат
            await this.showAnimatedGameResult(
                ctx, user, 'animated_darts', '🎯',
                dartsValue, betAmount, winAmount, resultText
            );
    ...
    private async playAnimatedBasketball(ctx: BotContext, betAmount: number): Promise<void> {
        const userId = ctx.from!.id;
        ...
        try {
            ...
            // Показываем результат
            await this.showAnimatedGameResult(
                ctx, user, 'animated_basketball', '🏀',
                basketballValue, betAmount, winAmount, resultText
            );
    ...
    private async playAnimatedBowling(ctx: BotContext, betAmount: number): Promise<void> {
        const userId = ctx.from!.id;
        ...
        try {
            ...
            // Показываем результат
            await this.showAnimatedGameResult(
                ctx, user, 'animated_bowling', '🎳',
                bowlingValue, betAmount, winAmount, resultText
            );
    ...
    private async processGameResult(
        ...
        try {
            ...
            const keyboard = Markup.inlineKeyboard([
                Markup.button.callback('🎮 Играть еще', 'play_again'),
        ...
        this.bot.command('create_task', async (ctx) => {
            ...
            if (!text.includes('"') || text.split('"').length < 5) {
                await ctx.reply(
                    ...
                    '<b>Пример:</b>\n' +
                    '<code>/create_task ref 25 "Посетите сайт" "Перейдите по ссылке" https://t.me/myballs/app?startapp=ref_npdo39sayc</code>',
                    { parse_mode: 'HTML' }
                );
                return;
            }
